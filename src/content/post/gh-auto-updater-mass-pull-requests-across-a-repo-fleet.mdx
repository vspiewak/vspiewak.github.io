...
**2. Fan-out topology.** One runner looping over 2,700 repos would take all night and die at the first hiccup. Instead : the parent workflow **chunks the target list (100 repos per chunk)** and fans out each chunk to a child workflow via a matrix (`fail-fast: false`) ; each child runs its repos as another matrix with `max-parallel: 5`. Hundreds of repos in flight, one job log per repo, one repo's failure hurting nobody else. **800+ repositories from a single launch**.

Here is what one manual trigger looks like — a fleet-wide `update-dataform-versions` run : chunked, fanned out, **818 jobs completed in under 17 minutes** ✅
...
**4. Respecting the rate limiter.** The real boss fight at scale is GitHub's **secondary rate limits** — the abuse-detection kind that a polite `sleep 5` does not appease. The demo's retry loop (10 attempts, 5s) grew into production's : *detect* rate-limit errors specifically, retry up to 50 times with **exponential backoff + jitter**, capped at 5 minutes :
...
That little block is the difference between "opened 400 PRs overnight" and "banned by abuse detection at PR #37" 😅
...
* **Label everything.** One label turned 27,652 PRs into a queryable dataset — metrics, audit trail and adoption tracking for free.
