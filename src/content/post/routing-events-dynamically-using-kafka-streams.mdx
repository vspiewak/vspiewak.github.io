...
After starting docker services using `docker compose up -d` and launching the SpringBoot application using `./mvnw clean spring-boot:run`, you can see that messages from `input-events` topic have been routed to the different country topics based on their field value.
